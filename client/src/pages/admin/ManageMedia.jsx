...
export default function ManageMedia() {
  ...
  const load = () =>
    request("/media", {
      params: { type, search: search || undefined, limit: 100 },
    })
      .then(setItems)
      .catch(() => setItems([]));
  ...
  const input =
    "w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 outline-none focus:border-accent/60 [&>option]:bg-ink-800";
  ...
  return (
    <div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-white/10">
        ...
        {items?.length === 0 && (
          <p className="bg-white/[0.02] p-10 text-center text-sm text-slate-500">
            No uploads yet.
          </p>
        )}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500">
                  {m.categoryName || ""} · {m.likesCount} likes · {m.views}{" "}
                  views · {timeAgo(m.createdAt)}
                </p>
              </div>
              <button
        ...
        {editing && (
          <>
            ...
            >
              ...
              <div className="mt-4 space-y-3">
                ...
                <label className="flex cursor-pointer items-center gap-2 text-sm text-accent-soft">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files?.[0] &&
                      setEditing({ ...editing, newThumb: e.target.files[0] })
                    }
                  />
                </label>
                {editing.newThumb && (
                  <img
                    src={URL.createObjectURL(editing.newThumb)}
                    alt="new thumbnail"
              ...
              <button
                type="submit"
                className="mt-5 w-full rounded-xl bg-gradient-to-r from-accent to-accent-blue py-2.5 text-sm font-semibold text-white"
              >
                Save changes
              </button>
            </motion.form>
          ...
          <>
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-400">
                "{confirmDelete.title}" will be permanently removed from
                Cloudinary and the database.
              </p>
              <div className="mt-5 grid grid-cols-2 gap-3">
                <button
                  onClick={() => setConfirmDelete(null)}
                  className="rounded-xl bg-white/10 py-2.5 text-sm font-medium text-slate-200"
                >
                  Cancel
                </button>
                <button
