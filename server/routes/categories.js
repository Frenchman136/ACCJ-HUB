...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(categories.map((c) => ({ ...c.toObject(), count: countMap[String(c._id)] || 0 })));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const maxOrder = await Category.findOne({ type }).sort('-order').lean();
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (inUse > 0)
      return res.status(400).json({ error: `Category has ${inUse} item(s). Move or delete them first.` });
